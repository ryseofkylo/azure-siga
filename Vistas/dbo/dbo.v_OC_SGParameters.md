---
esquema: dbo
tabla: v_OC_SGParameters
objeto: dbo.v_OC_SGParameters
tipo_objeto: VIEW
...
| 5 | `provincia` | varchar | 88% |
| 6 | `page_name` | varchar | 0% |
| 7 | `date_lead` | datetime | 0% |
| 8 | `Source` | nvarchar | 0% |
| 9 | `Campaign` | nvarchar | 0% |
| 10 | `producto` | varchar | 88% |
| 11 | `CasoId` | bigint | 0% |

## Definición (CREATE VIEW)
```sql
-- Vista: dbo.v_OC_SGParameters
-- Extraida: 2026-08-07T15:28:06.187640+00:00
-- Fuente: sys.sql_modules / OBJECT_DEFINITION

CREATE VIEW [dbo].[v_OC_SGParameters]
AS with datos as(
       SELECT distinct OC_Casoid AS Casoid
             FROM SG_OC_Parameter
), leadsSG as (
...
FROM   
       (SELECT OC_Casoid AS Casoid, [Key], cast([value] as varchar(1000)) as valor
             FROM SG_OC_Parameter
                    where OC_Casoid in (select Casoid from datos)
             ) p  
       PIVOT  
       (  
             max (valor)  
