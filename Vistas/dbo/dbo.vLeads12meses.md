...
```sql
-- Vista: dbo.vLeads12meses
-- Extraida: 2026-08-07T15:28:32.774776+00:00
-- Fuente: sys.sql_modules / OBJECT_DEFINITION

CREATE VIEW [dbo].[vLeads12meses]
AS WITH casos_filtrados AS (
    SELECT *
    FROM sg_caso
    WHERE Creation_date >= DATEADD(MONTH, -12, CAST(GETDATE() AS DATE))
)



SELECT
    c.[Id], c.[Source], c.[Creation_date], c.[Close_date], c.[Client_id], c.[Client_crm_id],
    c.[Client_name], c.[Client_ext_id], c.[File], c.[FechaProcesado], c.[NroCliente],
    c.[Campaign], c.[Closed_by], p.producto, p.utm_source, p.utm_medium, p.utm_campaign, p.provincia,
    'Unbounce BANNER WEB' AS TipoLead
FROM casos_filtrados c
LEFT JOIN v_SGParameters_materializada p ON c.id = p.casoid
LEFT JOIN SG_Tag t ON t.CasoId = c.id
WHERE p.utm_source IN ('Sup-WP')
  AND p.utm_medium IN ('Bnr')
  -- Opuesto a la condición de Widget WhatsApp: excluir los nombres problemáticos cuando exista tag
  AND (t.Name IS NULL OR LTRIM(RTRIM(t.Name)) NOT IN ('Chat2Whatsapp Deflection', 'Ventas Widget'))
  AND c.id NOT IN (SELECT DISTINCT Id FROM sg_caso)
