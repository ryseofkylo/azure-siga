...
| 13 | `producto` | varchar | 100% |
| 14 | `TipoLead` | varchar | 48% |

## Definición (CREATE VIEW)
```sql
-- Vista: dbo.v_OC_Leads_v2
-- Extraida: 2026-08-07T15:28:05.850893+00:00
-- Fuente: sys.sql_modules / OBJECT_DEFINITION

CREATE VIEW [dbo].[v_OC_Leads_v2]
...
-- Unbounce PREMIUM
select c.*, p.producto ,'Unbounce PREMIUM' as TipoLead
from sg_oc_caso c
left join v_oc_sgparameters p on c.id = p.casoid
where 
...
-- PAID MEDIA
select c.*, p.producto ,'Unbounce PAID MEDIA' as TipoLead
from sg_oc_caso c
left join v_oc_sgparameters p on c.id = p.casoid
where 
...
-- EMBEBIDAS
select c.*, p.producto ,'Unbounce EMBEBIDAS' as TipoLead
from sg_oc_caso c
left join v_oc_sgparameters p on c.id = p.casoid
where 
c.Campaign = 'Unbounce'
...
-- PBU
select c.*, p.producto ,'Unbounce PBU' as TipoLead
from sg_oc_caso c
left join v_oc_sgparameters p on c.id = p.casoid
where 
c.Campaign = 'Unbounce'
...
-- Comentarios Redes
select c.*, p.producto ,'Comentarios Redes' as TipoLead
from sg_oc_caso c
left join v_oc_sgparameters p on c.id = p.casoid
where 
...
select c.*, p.producto ,'Comentarios Anuncios' as TipoLead
from sg_oc_caso c
left join v_oc_sgparameters p on c.id = p.casoid
where 
...
-- Lead Ads
select c.*, p.producto ,'Lead Ads' as TipoLead
from sg_oc_caso c
left join v_oc_sgparameters p on c.id = p.casoid
where 
...
-- Email Marketing
select c.*, p.producto ,'Email Marketing' as TipoLead
from sg_oc_caso c
left join v_oc_sgparameters p on c.id = p.casoid
where 
...
-- WordPress
select c.*, p.producto ,'WordPress' as TipoLead
from sg_oc_caso c
left join v_oc_sgparameters p on c.id = p.casoid
where 
c.Campaign = 'WP Leads'
...
-- Botón de WhatsApp
select c.*, p.producto ,'Botón de WhatsApp' as TipoLead
from sg_oc_caso c
left join v_oc_sgparameters p on c.id = p.casoid
where 
c.Campaign = 'Whatsapp'
...
-- Forms Google Ads
select c.*, p.producto ,'Forms Google Ads' as TipoLead
from sg_oc_caso c
left join v_oc_sgparameters p on c.id = p.casoid
where 
...
-- WhatsApp Meta 
select c.*, p.producto ,'WhatsApp Meta' as TipoLead
from sg_caso c
left join v_oc_sgparameters p on c.id = p.casoid
where 
...
-- Click to WhatsApp
select c.*, p.producto ,'Click to WhatsApp' as TipoLead
from sg_oc_caso c
left join v_oc_sgparameters p on c.id = p.casoid
where 
c.Campaign = 'Whatsapp'
...
SELECT c.*, p.producto, 'Ventas Widget' AS TipoLead
