---
esquema: dbo
tabla: V_ORD_PENDIENTES_INDICADORES
objeto: dbo.V_ORD_PENDIENTES_INDICADORES
tipo_objeto: VIEW
...
| 4 | `FECHAFINALIZADA` | datetime2 | 80% |
| 5 | `HORAFINALIZADA` | datetime2 | 98% |
| 6 | `FECHAPROCESADA` | datetime2 | 48% |
| 7 | `HORAPROCESADA` | datetime2 | 100% |
| 8 | `FECHAAGENDADA` | datetime2 | 44% |
| 9 | `DEMORATOTAL` | int | 48% |
| 10 | `DEMORAPENDIENTE` | int | 0% |

## Definición (CREATE VIEW)
```sql
-- Vista: dbo.V_ORD_PENDIENTES_INDICADORES
-- Extraida: 2026-08-07T15:28:06.847692+00:00
-- Fuente: sys.sql_modules / OBJECT_DEFINITION

CREATE VIEW [dbo].[V_ORD_PENDIENTES_INDICADORES]
AS SELECT DISTINCT
	r.TAREAID, 
	...
	CASE WHEN ( a.demorapendiente IS NOT NULL )	THEN a.demorapendiente	ELSE b.demorapendiente	END AS DEMORAPENDIENTE

FROM SIGASC.ORDENES_PENDIENTES r
LEFT JOIN ( SELECT p.TAREAID, 
				...
				AVG(DATEDIFF(DAY,p.fechaingreso,GETDATE()))	AS DEMORAPENDIENTE
