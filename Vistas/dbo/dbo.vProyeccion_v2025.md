---
esquema: dbo
tabla: vProyeccion_v2025
objeto: dbo.vProyeccion_v2025
tipo_objeto: VIEW
...
```sql
-- Vista: dbo.vProyeccion_v2025
-- Extraida: 2026-08-07T15:28:34.147992+00:00
-- Fuente: sys.sql_modules / OBJECT_DEFINITION

CREATE VIEW [dbo].[vProyeccion_v2025]
AS SELECT 
...
-----<<INT>>------
SELECT 
[empresaid],[clientenro],[contratonro]
,SUM(
    CASE 
        WHEN PROMOID BETWEEN 70000 AND 70022 THEN 0   -- Skeelo
        WHEN PROMOID BETWEEN 11996 AND 11999 THEN 0   -- RDP
        ELSE IMPORTE_LINEA
    END
) AS FAC
,MAX(
 CASE 
   WHEN PROMOID BETWEEN 70000 AND 70022 THEN 0
   WHEN PROMOID BETWEEN 11996 AND 11999 THEN 0
   ELSE PROMOID
 END
) AS PROMOID
...
where [PERIODO] = CAST(FORMAT(DATEADD(MONTH, 0, GETDATE()), 'yyyyMM') AS INT)
AND [facturatpo]= 'F'
AND [facturagen]= 'M'
...
CASE WHEN D.PROMOID >0 THEN D.CUOTA END AS CUOTA, P.IMPORTE_LINEA AS MONTO_POLITICA
FROM (
SELECT [empresaid],[clientenro],[contratonro]
,SUM(
    CASE 
        WHEN PROMOID BETWEEN 70000 AND 70022 THEN 0   -- Skeelo
        WHEN PROMOID BETWEEN 11996 AND 11999 THEN 0   -- RDP
...
AND [facturagen]= 'M'
AND [PRODUCTOTPO] in ('M')
AND productoid NOT in (6402, 6418, 70048, 70061, 70044, 70074,70083, 6409,70049,70050,70062,70063) 
...
AND [PRODUCTOTPO] in ('Y')
   AND concepto NOT LIKE '%conex%'
      AND concepto NOT LIKE '%instal%'
      AND concepto NOT LIKE '%cargo%'
      AND concepto NOT LIKE '%abono adic%'
...
,CASE 
    ...
    ELSE 'OLD'
    END AS TIPO_POLITICA
,'SKEELO DESC'  AS TIPO_PROD
FROM [dbo].[BI_FACTURA_DETALLE_ALL]
where [PERIODO] = CAST(FORMAT(DATEADD(MONTH, 0, GETDATE()), 'yyyyMM') AS INT)
AND [facturatpo]= 'F'
AND [facturagen]= 'M'
AND ([PROMOID] between 70000 and 70022 or promoid between 11996 and 11999)
   AND concepto NOT LIKE '%conex%'
      AND concepto NOT LIKE '%instal%'
      AND concepto NOT LIKE '%cargo%'
      AND concepto NOT LIKE '%abono adic%'
...
AND PRODUCTONOMBRE not LIKE ('%MIO%')
