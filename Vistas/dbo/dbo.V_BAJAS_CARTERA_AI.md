---
esquema: dbo
tabla: V_BAJAS_CARTERA_AI
objeto: dbo.V_BAJAS_CARTERA_AI
tipo_objeto: VIEW
...
| 1 | `contratonro` | int | 0% |
| 2 | `clientenro` | int | 0% |
| 3 | `contratosts` | nvarchar | 0% |
| 4 | `empresaid` | int | 0% |
| 5 | `Fecha` | date | 0% |

## Definición (CREATE VIEW)
```sql
-- Vista: dbo.V_BAJAS_CARTERA_AI
-- Extraida: 2026-08-07T15:27:36.699099+00:00
-- Fuente: sys.sql_modules / OBJECT_DEFINITION

CREATE VIEW [dbo].[V_BAJAS_CARTERA_AI]
...
FROM (
  SELECT 
    ...
    contratosts, 
    bdmodifieddate, 
    LAG(contratosts) OVER (PARTITION BY contratonro, empresaid,clientenro ORDER BY bdmodifieddate) AS prev_contratosts
  FROM contratos_fixeados
  where BDMODIFIEDDATE is not null
) t
WHERE (contratosts != prev_contratosts OR prev_contratosts IS NULL))
-----------------------------------------------------------------------------
 SELECT contratonro,clientenro, contratosts,empresaid,DATEADD(month,1,DATEADD(year, -1,dateadd(day, 1, eomonth(GETDATE(), -1)))) as Fecha
    FROM (
        SELECT contratonro,clientenro, contratosts, bdmodifieddate,empresaid,
...
union
----------------------------------------
 SELECT contratonro,clientenro, contratosts,empresaid,DATEADD(month,2,DATEADD(year, -1,dateadd(day, 1, eomonth(GETDATE(), -1)))) as Fecha
    FROM (
        SELECT contratonro,clientenro, contratosts, bdmodifieddate,empresaid,
               ROW_NUMBER() OVER (PARTITION BY contratonro,empresaid ORDER BY bdmodifieddate DESC) AS rn
        FROM contratos_con_lag
...
union
----------------------------------------
 SELECT contratonro,clientenro, contratosts,empresaid,DATEADD(month,3,DATEADD(year, -1,dateadd(day, 1, eomonth(GETDATE(), -1)))) as Fecha
    FROM (
        SELECT contratonro,clientenro, contratosts, bdmodifieddate,empresaid,
               ROW_NUMBER() OVER (PARTITION BY contratonro,empresaid ORDER BY bdmodifieddate DESC) AS rn
        FROM contratos_con_lag
...
union
----------------------------------------
 SELECT contratonro,clientenro, contratosts,empresaid,DATEADD(month,4,DATEADD(year, -1,dateadd(day, 1, eomonth(GETDATE(), -1)))) as Fecha
    FROM (
        SELECT contratonro,clientenro, contratosts, bdmodifieddate,empresaid,
               ROW_NUMBER() OVER (PARTITION BY contratonro,empresaid ORDER BY bdmodifieddate DESC) AS rn
        FROM contratos_con_lag
...
union
----------------------------------------
 SELECT contratonro,clientenro, contratosts,empresaid,DATEADD(month,5,DATEADD(year, -1,dateadd(day, 1, eomonth(GETDATE(), -1)))) as Fecha
    FROM (
        SELECT contratonro,clientenro, contratosts, bdmodifieddate,empresaid,
               ROW_NUMBER() OVER (PARTITION BY contratonro,empresaid ORDER BY bdmodifieddate DESC) AS rn
        FROM contratos_con_lag
...
union
----------------------------------------
 SELECT contratonro,clientenro, contratosts,empresaid,DATEADD(month,6,DATEADD(year, -1,dateadd(day, 1, eomonth(GETDATE(), -1)))) as Fecha
    FROM (
        SELECT contratonro,clientenro, contratosts, bdmodifieddate,empresaid,
               ROW_NUMBER() OVER (PARTITION BY contratonro,empresaid ORDER BY bdmodifieddate DESC) AS rn
        FROM contratos_con_lag
...
union
----------------------------------------
 SELECT contratonro,clientenro, contratosts,empresaid,DATEADD(month,7,DATEADD(year, -1,dateadd(day, 1, eomonth(GETDATE(), -1)))) as Fecha
    FROM (
        SELECT contratonro,clientenro, contratosts, bdmodifieddate,empresaid,
               ROW_NUMBER() OVER (PARTITION BY contratonro,empresaid ORDER BY bdmodifieddate DESC) AS rn
        FROM contratos_con_lag
...
union
----------------------------------------
 SELECT contratonro,clientenro, contratosts,empresaid,DATEADD(month,8,DATEADD(year, -1,dateadd(day, 1, eomonth(GETDATE(), -1)))) as Fecha
    FROM (
        SELECT contratonro,clientenro, contratosts, bdmodifieddate,empresaid,
               ROW_NUMBER() OVER (PARTITION BY contratonro,empresaid ORDER BY bdmodifieddate DESC) AS rn
        FROM contratos_con_lag
...
union
----------------------------------------
 SELECT contratonro,clientenro, contratosts,empresaid,DATEADD(month,9,DATEADD(year, -1,dateadd(day, 1, eomonth(GETDATE(), -1)))) as Fecha
    FROM (
        SELECT contratonro,clientenro, contratosts, bdmodifieddate,empresaid,
               ROW_NUMBER() OVER (PARTITION BY contratonro,empresaid ORDER BY bdmodifieddate DESC) AS rn
        FROM contratos_con_lag
...
union
----------------------------------------
 SELECT contratonro,clientenro, contratosts,empresaid,DATEADD(month,10,DATEADD(year, -1,dateadd(day, 1, eomonth(GETDATE(), -1)))) as Fecha
    FROM (
        SELECT contratonro,clientenro, contratosts, bdmodifieddate,empresaid,
               ROW_NUMBER() OVER (PARTITION BY contratonro,empresaid ORDER BY bdmodifieddate DESC) AS rn
        FROM contratos_con_lag
...
union
----------------------------------------
 SELECT contratonro,clientenro, contratosts,empresaid,DATEADD(month,11,DATEADD(year, -1,dateadd(day, 1, eomonth(GETDATE(), -1)))) as Fecha
    FROM (
        SELECT contratonro,clientenro, contratosts, bdmodifieddate,empresaid,
               ROW_NUMBER() OVER (PARTITION BY contratonro,empresaid ORDER BY bdmodifieddate DESC) AS rn
        FROM contratos_con_lag
...
union
----------------------------------------
  SELECT contratonro,clientenro, contratosts,empresaid,DATEADD(month,12,DATEADD(year, -1,dateadd(day, 1, eomonth(GETDATE(), -1)))) as Fecha
    FROM (
        SELECT contratonro,clientenro, contratosts, bdmodifieddate,empresaid,
               ROW_NUMBER() OVER (PARTITION BY contratonro,empresaid ORDER BY bdmodifieddate DESC) AS rn
        FROM contratos_con_lag
